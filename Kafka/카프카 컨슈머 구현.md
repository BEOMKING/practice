...
단일 스레드로(위 그림의 `Listener1-0-C-1`)로 동작하기 때문에 1000개의 메시지를 수신하는데 약 10초가 소요되어 성능이 나쁜 것을 확인할 수 있다.
...
위 KafkaConsumerConfig 파일에 Concurrency 옵션을 추가하자.
...
위 사진에는 2개 밖에 없지만 총 5개의 컨슈머로 동작하며 더 빠른 속도로 메시지를 수신하게 되었다. 다만, 수신하는 메시지의 순서를 보장받지는 못한다.
